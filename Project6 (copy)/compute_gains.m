...
P.kp_phi = delta_a_max/e_phi_max*sign(a_phi2)*0;
P.kd_phi = (2*zeta_phi*wn_phi-a_phi1)/a_phi2*0;
...
P.kp_chi = 2*zeta_chi*wn_chi*Vg/P.g*0;
P.ki_chi = wn_chi^2*Vg/P.g*0;
...
P.kp_beta = delta_r_max/e_beta_max*sign(a_beta2)*0;
P.ki_beta = 1/a_beta2*((a_beta1 + a_beta2*P.kp_beta)/2/zeta_beta)^2*0;

%% Pitch Attitude Hold Gains
delta_e_max = 45*pi/180;

%%%DESIGN PARAMETERS%%%%%%
e_theta_max = 45*pi/180;
zeta_theta = .75;
%%%END%%%%%%%%%%%%%%%%%%%%
...
P.kd_theta = -0.77; %-(2*zeta_theta*wn_theta - a_theta1)/a_theta3;
...
P.ki_h = wn_h^2/(P.K_theta_DC*Va)*0;
P.kp_h = 2*zeta_h*wn_h/(P.K_theta_DC*Va)*0;
...
alpha_trim = atan(0/Va); %atan(x_trim(6)/x_trim(4));
...
P.ki_V = wn_V^2/a_V2*0;
P.kp_V = (2*zeta_V*wn_V - a_V1)/a_V2*0;